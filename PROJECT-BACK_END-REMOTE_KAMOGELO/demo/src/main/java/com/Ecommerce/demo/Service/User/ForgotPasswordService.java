...
public class ForgotPasswordService extends BaseService {
    ...
    private void sendOTP(User user) {
        ...
        if (optionalForgotPassword.isEmpty()) {
            ...
            publishOTPViaEmail(user); //we create a new opt and send
            return;
        }
        ...
        if (minutesPassed > 10) {
            ...
            publishOTPViaEmail(user); //we create a new one since the old one has expired
        }
    }

    @Async
    private void publishOTPViaEmail(User user) {
